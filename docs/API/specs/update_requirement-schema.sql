...
CREATE TABLE requirement."RequirementTemplateVersions" (
  ...
  -- [v2] Concurrency guard on template configuration (UX-08). Two authors
  -- editing the same draft version must not silently overwrite each other.
...
-- =====================================================================
-- =====================================================================
-- [v2] requirement.RequirementLineVisibility
-- Disclosure control for the resolved contract and the explain view.
-- UX-03 requires three audiences (customer minimal, provider later,
-- dispatcher all); UX-04 requires knowing which changes are customer-visible
-- so approval can be scoped to those alone.
--
-- Held as one keyed table rather than three columns repeated across every
-- requirement-line table: explain assembles lines from many sources, and a
-- single lookup keeps the filter in one place.
-- =====================================================================
CREATE TYPE requirement.visibility_level AS ENUM (
  'Customer','Provider','Internal'
);

CREATE TABLE requirement."RequirementLineVisibility" (
  "Id"                uuid          NOT NULL,
  "TenantId"          uuid          NOT NULL,
  "WorkRequirementId" uuid          NOT NULL,
  "LineType"          varchar(60)   NOT NULL,                         -- e.g. CapabilityRequirement
  "LineId"            uuid          NOT NULL,                         -- the row in that table
  "VisibilityLevel"   requirement.visibility_level NOT NULL DEFAULT 'Internal',
  "CustomerVisible"   boolean       NOT NULL DEFAULT false,           -- drives approval scope (UX-04)
  CONSTRAINT "PK_RequirementLineVisibility" PRIMARY KEY ("Id"),
  CONSTRAINT "FK_RLV_WR"
    FOREIGN KEY ("WorkRequirementId") REFERENCES requirement."WorkRequirements" ("Id")
    ON DELETE CASCADE,
  CONSTRAINT "UQ_RLV_Line" UNIQUE ("LineType","LineId"),
  -- Customer-visible implies at least Customer disclosure level.
  CONSTRAINT "CK_RLV_Consistent"
    CHECK ("CustomerVisible" = false OR "VisibilityLevel" = 'Customer')
);
CREATE INDEX "IX_RLV_WR"
  ON requirement."RequirementLineVisibility" ("TenantId","WorkRequirementId","VisibilityLevel");
